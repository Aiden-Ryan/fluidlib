...
def TMatrix(nodes):
    ...
    '''
    array1 = [node.T for node in nodes]
    array2 = [node.rho for node in nodes]
    array3 = [node.c for node in nodes]
    array4 = [node.q for node in nodes]
    array5 = [node.Eg for node in nodes]
    array6 = [node.Tinf for node in nodes]
    array7 = [node.h for node in nodes]
    array8 = [node.e for node in nodes]
    array9 = [node.Tsurr for node in nodes]
    array10 = [node.Ac for node in nodes]
    array11 = [node.Ar for node in nodes]
    array12 = [node.V for node in nodes]
...
def ThermalSolve():
    ...
    paths = np.empty(n-1, dtype=Path)
    print("ENTER NODE ATTRIBUTES. ENTER 0 TO IGNORE PARAMETER. \n")
    if n == 1:
            ...
            print('\n')
            nodes[0] = Node(T,Ac, Ar, rho, c, q, Eg, h, Tinf, Tsurr, e, mat,V)
    else:
        for i in range(n):
            T = float(input("ENTER TEMPERATURE OF LUMPED NODE: "))
            Ac = float(input("ENTER CONVECTIVE AREA OF LUMPED NODE: "))
            ...
            mat = input("ENTER MATERIAL: ")
            print('\n')
            nodes[i] = Node(T,Ac, Ar, rho, c, q, Eg, h, Tinf, Tsurr, e, mat,V)
